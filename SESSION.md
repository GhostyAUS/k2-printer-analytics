...
- **Thumbnails**: Creality Print does NOT embed thumbnails in standalone `.gcode` files (metadata always returns `slicer: Unknown`, `thumbnails: []`). Thumbnails only exist inside `.3mf` project directories as `{base}_plate_{N}.png` files. The `_find_thumbnail_path()` function searches metadata → matching PNG in same 3MF dir → scan all 3MF dirs for matching gcode. `thumbnail_path` is saved in DB when a job is created so it persists if the file is deleted from the printer.
